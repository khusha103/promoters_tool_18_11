...
photos: { url: string; caption?: string }[] = []; // Array to hold uploaded photos
temporaryCaptions: string[] = []; // Array to hold temporary captions
feedback: string = ''; // Variable to hold user feedback
categoryId: string = ''; // Store the categoryId from query params
evalId: string = ''; // Store the evalId from query params
pagesource: string = '';
...
async captureImage(source: CameraSource) {
  try {
    ...
    if (image && image.webPath) {
      const fileUri = image.webPath; // Use webPath for displaying the image
      this.photos.push({ url: fileUri, caption: '' }); // Add photo with an empty caption
      this.temporaryCaptions.push(''); // Initialize corresponding temporary caption as empty
    }
  } catch (error) {
...
submitFeedback() {
  ...
  // Append basic data (feedback, categoryId, evalId)
  // this.categoryId = "4";
  // this.evalId = "3043";
...
// Function to send the FormData to the server
