...
//   constructor(private router: Router, private route: ActivatedRoute, private apiService: ApiService, private iab: InAppBrowser, private cd: ChangeDetectorRef) { }
...
export class ProductInnerPage implements OnInit {
  ...
  openPdf(pdf: any) {
  // Use '_blank' so the InAppBrowser UI is used (not the external system browser)
  const options = 'hidden=no,location=no,toolbar=no,hardwareback=yes,fullscreen=yes,zoom=no';
  const browser = this.iab.create(pdf.url, '_blank', options);

  // if you used hidden=yes above, you could call browser.show() after loadstop.
  browser.on('loadstop').subscribe(() => {
    // optionally show or run JS if needed
    // browser.show(); // uncomment if you used hidden=yes
  });
...
openVideo(video: any) {
  const options = 'hidden=no,location=no,toolbar=no,hardwareback=yes,fullscreen=yes,zoom=no';
  const browser = this.iab.create(video.url, '_blank', options);
