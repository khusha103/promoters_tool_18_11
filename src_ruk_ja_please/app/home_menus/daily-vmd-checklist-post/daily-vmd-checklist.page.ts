...
@Component({
  ...
  templateUrl: './daily-vmd-checklist.page.html',
  styleUrls: ['./daily-vmd-checklist.page.scss'],
})
export class DailyVmdChecklistPage implements OnInit {
  countries: any[] = [];
  retailers: any[] = [];
  alloutlets: any[] = [];
  allretailers: any[] = [];
  ...
  //   if (categoryId && countryId && storeId) {
  //     this.router.navigate(['/daily-vmdchecklist-ques'], {
  //       queryParams: { categoryId, countryId, storeId }
  //     });
  //   } else {
  //     // Handle cases where one or more selections are missing
